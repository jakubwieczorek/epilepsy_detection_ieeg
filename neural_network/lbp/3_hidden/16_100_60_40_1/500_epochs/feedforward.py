...
class FeedForward:
    ...
    def load(cls, path):
        ...
        if not isinstance(w4[0], list): # if only one output, then wrap it into an array
                                        # so [1,2,3] -> [[1],[2],[3]]            
            w4 = [[w4[i]] for i in range(len(w4))]

        if not isinstance(b4, list): # if only one output, then wrap it into an array
            b4 = [b4]
    ...
    def think(self, x):
        ...
        (self.output, self.output_z) = self.feedForward(self.l3_output, self.w4, self.b4, self.sigmoid)
        return self.output
...
if __name__ == "__main__":
    ...
    data = parse_csv("../../../dataset/ID02/16/test_data.csv", 16)
    ...
    data = parse_csv("../../../dataset/ID02/16/train_data.csv", 16)
