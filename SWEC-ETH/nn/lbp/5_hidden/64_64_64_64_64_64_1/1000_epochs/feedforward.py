...
class FeedForward:
    def __init__(self, w1, b1, w2, b2, w3, b3, w4, b4, w5, b5, w6, b6):
        self.w1 = w1
        self.b1 = b1
        self.w2 = w2
        self.b2 = b2
    ...
    def load(cls, path):
        ...
        w6 = np.loadtxt(path + '/w6.txt', dtype=float).tolist()
        b6 = np.loadtxt(path + '/b6.txt', dtype=float).tolist()

        if not isinstance(w6[0], list): # if only one output, then wrap it into an array
                                        # so [1,2,3] -> [[1],[2],[3]]            
            w6 = [[w6[i]] for i in range(len(w6))]

        if not isinstance(b6, list): # if only one output, then wrap it into an array
            b6 = [b6]
        
        return cls(w1, b1, w2, b2, w3, b3, w4, b4, w5, b5, w6, b6)
    ...
    def think(self, x):
        ...
        (self.l5_output, self.l5_z)  = self.feedForward(self.l4_output, self.w5, self.b5, self.ReLU)
        (self.output, self.output_z) = self.feedForward(self.l5_output, self.w6, self.b6, self.sigmoid)
        return self.output
...
if __name__ == "__main__":
    ...
    data = parse_csv("../../../dataset/ID02/64/all_sensors/test_data.csv", 64)
    ...
    data = parse_csv("../../../dataset/ID02/64/all_sensors/train_data.csv", 64)
